...
class FASS {
  constructor(options) {
    this.options = _.defaults(options, this.getDefaults());
    ...
    if (this.options.upload && !this.options.uploadBucket) {
      this.quit('FASS requires a uploadBucket parameter if upload is set to true.');
    }
  ...
  shoot() {
    ...
    screenshot(screenshotPath, (error, complete) => {
      ...
      else if (this.options.upload) {
        this.upload(screenshotPath);
      }
    });
  }

  cv(screenshotPath) {
    const metaPath = screenshotPath.replace('.png', '.vision.json');
  ...
  upload(screenshotPath, metaPath) {
    try {
      ...
      const metaFile = metaPath ? metaPath.split('/').pop() : null;
      const screenshotKey = [this.options.uploadPrefix, screenshotFile].join('/').replace(/\/\//g, '/');
      const metaKey = metaPath ? [this.options.uploadPrefix, metaFile].join('/').replace(/\/\//g, '/') : null;
      ...
      const uploader = new AWS.S3({ params: {
        Bucket: this.options.uploadBucket
      }});
